...
function Room() {
  ...
  const { id } = useParams();
  const [changeTreatingDays, { isLoading: isChanging }] =
    useChangeTreatingDaysMutation();
  const [removePatientFromRoom, { isLoading: isRemoving }] =
    useRemovePatientFromRoomMutation();
  ...
  const patients = useMemo(
    () =>
      roomData?.innerData?.capacity?.map((item) => ({
        ...
        choosedRoomServices: item.choosedRoomServices,
        startDay: item.startDay,
        payForRoom: item.roomId?.pricePerDay || 0,
        paidDays: item.paidDays || [],
        clientPayForRoomPrice: calculateRoomPayment(item),
        services: item.choosedRoomServices?.services,
        roomNumber: item.roomId?.roomNumber,
        clientServicePrice: item.choosedRoomServices?.services
          ?.map(value => value?.serviceId?.price || 0)
  ...
  const exitRoom = useCallback(
    async (record) => {
      try {
        if (!id || !record.clientMongooseId)
          throw new Error("Kerakli ma'lumotlar mavjud emas");
        await removePatientFromRoom({ id, patientId: record._id }).unwrap();
        toast.success(
          `${record.clientFullname} xonadan muvaffaqiyatli chiqarildi!`
        );

        if (patients.length === 1) {
          toast.info("Xonada boshqa bemorlar qolmadi. Orqaga qaytilmoqda...");
          setTimeout(() => navigate(-1), 2000);
        }
      } catch (error) {
        toast.error(
          error?.data?.message ||
          "Bemorni xonadan chiqarishda xatolik yuz berdi!"
        );
      } finally {
  ...
  const handlePayment = useCallback(async () => {
    if (!paymentAmount || !currentPatient) {
      toast.warning("Iltimos, to'lov miqdorini kiriting.");
      return;
    }
    ...
    try {
      ...
      setSendPrint({
        ...
        currentPatient,
        currentPatient,
        servicePrice: currentPatient?.roomStoryId.clientServicePrice,
      });
      await refetch();
  ...
  const handleDayManagement = useCallback(
    async (action) => {
      ...
      try {
        ...
        toast.success(
          `Kun muvaffaqiyatli ${action.type === "inc" ? "qo'shildi" : "olib tashlandi"
          }!`
        );
  ...
  const calculateDebt = useCallback((record) => {
    ...
    const roomDebt = record.paidDays
      .filter((day) => !day.isPaid)
      .reduce((sum, day) => sum + (record.payForRoom - (day.price || 0)), 0);

    // Add clientServicePrice to the total debt, default to 0 if undefined
    return roomDebt + (record?.clientServicePrice || 0);
  }, []);
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        render: (price) => `${formatNumber(price || 0)} so'm`,
      },
      {
        title: "Doktor",
        dataIndex: "doctorId",
      ...
      {
        title: "Qo'shimcha xizmatlar narxi",
        dataIndex: "clientServicePrice",
        render: (price) => `${formatNumber(price || 0)} so'm`,
      },
      {
        ...
        width: 140,
        render: (record) => {
          const hasUnpaidDays = record.paidDays?.some(
            (day) => !day.isPaid && (day.price || 0) > 0
          );
          return (
            <Tooltip
              title={
      ...
      <div className="my-table-container">
        <div className="my-table-box">
          {(record.paidDays || []).map((item, i) => {
            const isToday = moment(item.date, "DD.MM.YYYY").isSame(
              moment(),
              "day"
            );
            ...
            return (
              <div
                key={i}
                className={getCardClassName(item.isPaid, item.price, isToday)}
  ...
  useEffect(() => {
    const handleUpdateRoomPayment = () =>
      console.log("Room payment updated via socket");
    const socket = {
      on: (event, callback) => console.log(`Socket event ${event} registered`),
    };
    ...
    () => [
      ...
      { label: "Sig'imi", value: roomInfo.capacity?.length || 0 },
      {
        ...
        value: roomInfo.pricePerDay
          ? `${formatNumber(roomInfo.pricePerDay)} so'm`
          : "N/A",
      },
    ],
  ...
  return (
    <div className="updateRoom_wrapper">
        ...
        <div className="custom-modal-overlay">
          <div className="custom-modal">
            <div className="custom-modal-box">
              ...
              <p>
                Bu amal bemorni xonadan olib tashlaydi. Davom etishni
                xohlaysizmi?
              </p>
                  ...
                  {modalType === "payment" ? (
                    <p className="debt-amount">
                      ...
                      <span className="debt-value">
                        {formatNumber(calculateDebt(currentPatient))} so'm
                      </span>
                    </p>
                      ...
                      {currentPatient.paidDays.map((value, inx) => {
                        ...
                        return (
                          <div key={inx} className={getDebtDetailsClass()}>
                            <span className="date-label">{value.date}</span>
                ...
                <div className="payment-form">
                  ...
                  <div className="form-group">
                    ...
                    <div className="payment-type-selector">
                      <label
                        className={`payment-option ${paymentType === "naqt" ? "selected" : ""
                          }`}
                      >
                        <input
                          type="radio"
                      ...
                      <label
                        className={`payment-option ${paymentType === "karta" ? "selected" : ""
                          }`}
                      >
                        <input
                          type="radio"
      ...
      <div style={{ display: "none" }}>
        <ReceiptPrint ref={receiptRef} data={sendPrint} room={roomInfo} />
      </div>
    </div>
  );
}

...
} from "../../../../context/clientApi";
import { useAddPatientToRoomMutation } from "../../../../context/roomApi";
import { useGetStoryByPatientAndDoctorQuery } from "../../../../context/storyApi";
...
const PatientAddRoomForm = () => {
  ...
  const filteredDoctors =
    doctors?.innerData?.filter((doctor) => doctor.role === "doctor") || [];
  ...
  const handleSaveIdNumber = async (patientId) => {
    ...
    try {
      ...
      toast.error(
        error.data.message || "ID Raqamni yangilashda xatolik yuz berdi!",
        {
          position: "top-center",
          autoClose: 3000,
        }
  ...
  const handleSubmit = async (patient, days) => {
    // Check if patient.idNumber is missing
    if (!patient.idNumber) {
      toast.warning("Bemorni pasport seria raqamini kiriting!", {
        position: "top-center",
        autoClose: 3000,
      });
      return;
    ...
    if (!patient || !id || !days || !selectedDoctor) {
      toast.warning(
        "Iltimos, barcha maydonlarni to'ldiring, shu jumladan shifokor!",
        {
          position: "top-center",
          autoClose: 3000,
        }
    ...
    const assignmentData = {
      ...
      doctorId: selectedDoctor,
      storiesId: stories?.innerData?._id
    };
    ...
    try {
      ...
      toast.error(
        error.data.message || "Xonaga joylashtirishda xatolik yuz berdi!",
        {
          position: "top-center",
          autoClose: 3000,
        }
    ...
    <div className="PatientAddRoomForm-container">
      ...
      <div className="PatientAddRoomForm-card">
        <div className="PatientAddRoomForm-header">
          ...
          <h1 className="PatientAddRoomForm-title">
            Bemorni Xonaga Joylashtirish
          </h1>
        </div>
            ...
            <table className="PatientAddRoomForm-table">
              <thead className="PatientAddRoomForm-tableHeader">
                <tr>
                  ...
                  <th className="PatientAddRoomForm-tableHeaderCell">
                    ID Raqam
                  </th>
                  <th className="PatientAddRoomForm-tableHeaderCell">
                    Telefon
              ...
              <tbody className="PatientAddRoomForm-tableBody">
                {filteredPatients.map((patient) => (
                  <tr
                    key={patient._id}
                    className={`PatientAddRoomForm-tableRow ${selectedPatient?._id === patient._id
                      ? "PatientAddRoomForm-tableRowSelected"
                      : ""
                      }`}
                  >
                    ...
                    <td className="PatientAddRoomForm-tableCell">
                      {isEditingId === patient._id ? (
                        ...
                        <div className="PatientAddRoomForm-cellContent">
                          <span>
                            {patient.idNumber || "ID Raqam kiritilmagan"}
                          </span>
                          {!patient.idNumber && (
                            <button
                    ...
                    <td className="PatientAddRoomForm-tableCell">
                      <div className="patientAddRoomForm-actions">
                        <button
                          ...
                          className={`PatientAddRoomForm-selectButton ${selectedPatient?._id === patient._id
                            ? "PatientAddRoomForm-selectButtonActive"
                            : ""
                            }`}

...
import { Button, Radio, Input, Form, Row, Select } from "antd";
...
const RoomForm = ({ visible, onClose, mode = "create", roomData = null }) => {
  ...
  const [updateRoomId, { isLoading: isUpdatingRoomId }] =
    useUpdateRoomIdMutation();
  ...
  const nurses =
    data?.innerData?.filter((worker) => worker.role === "nurse") || [];
  const cleaners =
    data?.innerData?.filter((worker) => worker.role === "cleaner") || [];
  const doctor =
    data?.innerData?.filter((worker) => worker.role === "doctor") || [];
  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      toast.warn(
        error.data?.message ||
          `Xona ${
            mode === "update" ? "yangilashda" : "qo'shishda"
          } xatolik yuz berdi`,
        {
          position: "top-right",
          autoClose: 3000,
  ...
  return (
    <div className="modalroom-content">
      ...
      >
        ...
        <Form.Item label="Xona turi" name="roomType" initialValue={roomType}>
          <Select
            showSearch
        ...
        <Form.Item
          ...
          rules={[
            {
              required:
                roomType === "davolanish" || roomType === "vrach_kabineti",
              ...
              {doctor.map((nurse) => (
                <Option key={nurse._id} value={nurse._id}>
                  {`${capitalizeFirstLetter(
                    ...
                    nurse.lastName
                  )}: ${capitalizeFirstLetter(nurse.specialization)}`}
                </Option>
              ))}
            </Select>
          </Form.Item>
          ...
          >
            {nurses.map((nurse) => (
              <Option key={nurse._id} value={nurse._id}>
                {`${capitalizeFirstLetter(
                  nurse.firstName
                )} ${capitalizeFirstLetter(nurse.lastName)}`}
              </Option>
            ))}
          </Select>
        </Form.Item>
          ...
          >
            {cleaners.map((cleaner) => (
              <Option key={cleaner._id} value={cleaner._id}>
                {`${capitalizeFirstLetter(
                  cleaner.firstName
                )} ${capitalizeFirstLetter(cleaner.lastName)}`}
              </Option>
            ))}
          </Select>
        </Form.Item>

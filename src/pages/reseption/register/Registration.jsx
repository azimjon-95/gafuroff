...
import {
  ...
  useLazyFindPatientByPhoneQuery,
...
const Registration = () => {
  ...
  const handlePhoneChange = async (e) => {
    ...
    if (cleanedValue.length === 9) {
      try {
        const res = await findPatientByPhone(cleanedValue).unwrap();
        if (res?.innerData) {
          const patient = res.innerData;
          form.setFieldsValue({
      ...
      >
              ...
              {doctors?.innerData?.length > 0 ? (
                doctors.innerData.map((doctor) => (
                  <Option key={doctor._id} value={doctor._id}>
                    ...
                    {doctor.firstName} {doctor.lastName} [{doctor.todayQueue}]
                  </Option>
                ))
              ) : (
        ...
        <Form.Item
          ...
          rules={[
            {
              required: true,
              message: "Iltimos, kamida bitta xizmat tanlang!",
            },
          ]}
        >
          <Select
            ...
            {getDoctorServices().length > 0 ? (
              getDoctorServices().map((service) => (
                <Option
                  ...
                  label={`${service.name} (${formatPaymentAmount(
                    service.price.toString()
                  )} soʻm)`}
                >
                  {service.name} (
                  {formatPaymentAmount(service.price.toString())} soʻm)
                </Option>
              ))
            ) : (
              <Option disabled value="">
        ...
        <div className="form-row">
          <Form.Item
            ...
            label="Ism"
            rules={[
              { required: true, message: "Iltimos, bemor ismini kiriting!" },
            ]}
          >
          ...
          <Form.Item
            ...
            rules={[
              {
                required: true,
                message: "Iltimos, bemor familiyasini kiriting!",
              },
            ]}
          >
        ...
        <div className="form-row">
          <Form.Item
            ...
            rules={[
              {
                required: true,
                message: "Iltimos, tugʻilgan yilni kiriting!",
              },
              {
                pattern: /^\d{4}$/,
                message:
                  "Tugʻilgan yil 4 raqamli boʻlishi kerak (masalan, 1990)!",
              },
            ]}
          >
        ...
        <div className="form-row">
          ...
          <Form.Item
            ...
            rules={[
              {
                required: true,
                message: "Iltimos, toʻlov summasini kiriting!",
              },
              {
                validator: (_, value) => {
                  if (!value) return Promise.resolve();
                  ...
                  return Promise.reject(
                    new Error(
                      "Toʻlov summasi faqat raqamlardan iborat boʻlishi kerak!"
                    )
                  );
                },
              },
          ...
          >
            <Input readOnly placeholder="50.000" />
          </Form.Item>
        </div>

  ...
  useEffect(() => {
    if (clinicToEdit) {
      ...
      form.setFieldsValue({
        ...
        work_schedule: {
          start_time: clinicToEdit.work_schedule?.start_time
            ? moment(clinicToEdit.work_schedule.start_time, "HH:mm")
            : null,
          end_time: clinicToEdit.work_schedule?.end_time
            ? moment(clinicToEdit.work_schedule.end_time, "HH:mm")
            : null,
          work_days: workDays,
          lunch_break: {
  ...
  const handleDayToggle = useCallback(
    (day) => {
      setSelectedDays((prev) => {
        ...
        form.setFieldsValue({
          work_schedule: {
            ...form.getFieldValue("work_schedule"),
            work_days: newDays,
          },
        });
        return newDays;
  ...
  const handleSubmit = useCallback(
    async (values) => {
      ...
      const formattedValues = {
        ...
        logo: logoUrl,
        work_schedule: {
          start_time: values.work_schedule?.start_time
            ? values.work_schedule.start_time.format("HH:mm")
            : "08:00",
          end_time: values.work_schedule?.end_time
            ? values.work_schedule.end_time.format("HH:mm")
            : "17:00",
          work_days: values.work_schedule?.work_days || [
            "monday",
            "tuesday",
            "wednesday",
            "thursday",
          ...
          lunch_break: {
            start_time: values.work_schedule?.lunch_break?.start_time
              ? values.work_schedule.lunch_break.start_time.format("HH:mm")
              : "12:00",
            end_time: values.work_schedule?.lunch_break?.end_time
              ? values.work_schedule.lunch_break.end_time.format("HH:mm")
              : "13:00",
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        render: (lunch_break) =>
          lunch_break.enabled
            ? `${lunch_break.start_time} - ${lunch_break.end_time}`
            : "Yo'q",
      },
      {
        title: "Manzil",
    ...
    <div style={{ padding: "24px", background: "#f0f2f5", minHeight: "100vh" }}>
          ...
          <Row gutter={16}>
            ...
            <Col xs={24} md={12}>
              <Form.Item
                ...
                name={["work_schedule", "start_time"]}
                rules={FORM_RULES.work_schedule_start_time}
              >
                <TimePicker
                  format="HH:mm"
                  style={{ width: "100%" }}
            ...
            <Col xs={24} md={12}>
              <Form.Item
                label="Tugash Vaqti"
                name={["work_schedule", "end_time"]}
                rules={FORM_RULES.work_schedule_end_time}
              >
                <TimePicker
                  format="HH:mm"
                  style={{ width: "100%" }}
            ...
            <Col xs={24} md={12}>
              <Form.Item
                ...
                name={["work_schedule", "lunch_break", "start_time"]}
                rules={FORM_RULES.lunch_break_start_time}
              >
                <TimePicker
                  format="HH:mm"
                  style={{ width: "100%" }}
            ...
            <Col xs={24} md={12}>
              <Form.Item
                ...
                name={["work_schedule", "lunch_break", "end_time"]}
                rules={FORM_RULES.lunch_break_end_time}
              >
                <TimePicker
                  format="HH:mm"
                  style={{ width: "100%" }}
            ...
            <Col xs={24}>
              <Form.Item
                name={["work_schedule", "lunch_break", "enabled"]}
      ...
      >
        {isLoading ? (
          ...
          <div>
            Klinikalarni yuklashda xatolik:{" "}
